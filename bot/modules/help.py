...
async def arg_usage(_, query):
    ...
    if data[1] == "nex":
        ...
        buttons = ButtonMaker()
        buttons.data_button(SFMLStyle.BACK_BT, f"help back m {pg_no}")
        button = buttons.build_menu()
        await edit_message(message, MIRROR_HELP_DICT[data[2]], button)
    elif data[1] == "yt":
        buttons = ButtonMaker()
        buttons.data_button(SFMLStyle.BACK_BT, f"help back y {pg_no}")
        button = buttons.build_menu()
        await edit_message(message, YT_HELP_DICT[data[2]], button)
    elif data[1] == "clone":
        buttons = ButtonMaker()
        buttons.data_button(SFMLStyle.BACK_BT, f"help back c {pg_no}")
        button = buttons.build_menu()
...
async def bot_help(_, message):
    await send_message(message, SFMLStyle.HELP_HEADER + "\n\n" + help_string)
