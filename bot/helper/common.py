...
from .ext_utils.rename_utils import autorename_exec
...
class TaskConfig:
    ...
    async def before_start(self):
        ...
        self.autorename = (
            self.autorename
            or self.user_dict.get("AUTORENAME", False)
            or (Config.AUTORENAME if "AUTORENAME" not in self.user_dict else "")
        )
        ...
        if self.is_leech and (
            ...
            for key, dict_name, tag in [
                ...
                ("METADATA_DUBBED_BY", "metadata_dict", "dubbed_by"),
                ("METADATA_CHANNEL", "metadata_dict", "channel"),
                ("METADATA_WEBSITE", "metadata_dict", "website"),
                ("METADATA_COPYRIGHT", "metadata_dict", "copyright"),
                ("METADATA_PUBLISHER", "metadata_dict", "publisher"),
                ("METADATA_ENCODER", "metadata_dict", "encoder"),
                ("METADATA_SOURCE", "metadata_dict", "source"),
                ("METADATA_STUDIO", "metadata_dict", "studio"),
            ]:
    ...
    async def substitute(self, dl_path):
        def perform_swap(name, swaps):
            ...
            for swap in swaps:
                if len(swap) == 1:
                    pattern, res, cnt, sen = swap[0], "", 0, ""
                elif len(swap) == 2:
                    pattern, res, cnt, sen = swap[0], swap[1], 0, ""
                elif len(swap) == 3:
                    if swap[2].isdigit():
                        pattern, res, cnt, sen = swap[0], swap[1], int(swap[2]), ""
                    else:
                        pattern, res, cnt, sen = swap[0], swap[1], 0, swap[2]
                elif len(swap) >= 4:
                    pattern, res, cnt, sen = (
                        swap[0],
                        swap[1],
                        int(swap[2]) if swap[2].isdigit() else 0,
                        swap[3],
                    )
                else:
                    continue
